 ...
 * @private
 */
var TpmSecurityClient = (function () {
    ...
    TpmSecurityClient.prototype._createPersistentPrimary = function (name, hierarchy, handle, template, callback) {
        var _this = this;
        this._tpm.allowErrors().ReadPublic(handle, function (resp) {
            var rc = _this._tpm.getLastResponseCode();
            ...
            if (rc !== tss_js_1.TPM_RC.SUCCESS) {
                ...
                _this._tpm.withSession(tss.NullPwSession).CreatePrimary(hierarchy, new tss.TPMS_SENSITIVE_CREATE(), template, null, null, function (resp) {
                    debug('CreatePrimary(' + name + ') returned ' + tss_js_1.TPM_RC[_this._tpm.getLastResponseCode()]);
                    _this._tpm.withSession(tss.NullPwSession).EvictControl(tss.Owner, resp.handle, handle, function () {
                        debug('EvictControl(0x' + resp.handle.handle.toString(16) + ', 0x' + handle.handle.toString(16) + ') returned ' + tss_js_1.TPM_RC[_this._tpm.getLastResponseCode()]);
                        _this._tpm.FlushContext(resp.handle, function () {
                            debug('FlushContext(TRANSIENT_' + name + ') returned ' + tss_js_1.TPM_RC[_this._tpm.getLastResponseCode()]);
    ...
    TpmSecurityClient.prototype._getPropsAndHashAlg = function (callback) {
        ...
        this._tpm.GetCapability(tss.TPM_CAP.TPM_PROPERTIES, tss_js_1.TPM_PT.INPUT_BUFFER, 1, function (caps) {
    ...
    TpmSecurityClient.prototype._signData = function (dataToSign, callback) {
        ...
        this._getPropsAndHashAlg(function (idKeyHashAlg, props) {
            if (props.tpmProperty.length !== 1 || props.tpmProperty[0].property !== tss_js_1.TPM_PT.INPUT_BUFFER) {
                ...
                if (dataToSign.length <= maxInputBuffer_1) {
                    _this._tpm.withSession(tss.NullPwSession).HMAC(TpmSecurityClient._idKeyPersistentHandle, dataToSign, idKeyHashAlg, function (signature) {
                    ...
                    var loopFn_1 = function () {
                        if (bytesLeft_1 > maxInputBuffer_1) {
                            ...
                            _this._tpm.withSession(tss.NullPwSession).SequenceComplete(hSequence_1, dataToSign.slice(curPos_1, curPos_1 + bytesLeft_1), new tss_js_1.TPM_HANDLE(tss.TPM_RH.NULL), function (resp) {
                                callback(null, resp.result);
                            });
                        }
                    };
                    _this._tpm.withSession(tss.NullPwSession).HMAC_Start(TpmSecurityClient._idKeyPersistentHandle, new Buffer(0), idKeyHashAlg, function (hSeq) {
                        hSequence_1 = hSeq;
                        loopFn_1();
                    });
                }
    ...
    TpmSecurityClient.prototype._activateIdentityKey = function (activationBlob, activateCallback) {
        ...
        //
        _a = tss.marshal.sizedFromTpm(tss.TPMS_ID_OBJECT, activationBlob, 2, currentPosition), credentialBlob = _a[0], currentPosition = _a[1];
        ...
        _b = tss.marshal.sizedFromTpm(tss_js_1.TPMT_PUBLIC, activationBlob, 2, currentPosition), this._idKeyPub = _b[0], currentPosition = _b[1];
        debug('idKeyPub end: ' + currentPosition);
        //
        // Start a policy session to be used with ActivateCredential()
        //
        this._tpm.GetRandom(TpmSecurityClient._tpmNonceSize, function (nonce) {
            _this._tpm.StartAuthSession(null, null, nonce, null, tss.TPM_SE.POLICY, tss.NullSymDef, tss_js_1.TPM_ALG_ID.SHA256, function (resp) {
                debug('StartAuthSession(POLICY_SESS) returned ' + tss_js_1.TPM_RC[_this._tpm.getLastResponseCode()] + '; sess handle: ' + resp.handle.handle.toString(16));
                if (_this._tpm.getLastResponseCode() !== tss_js_1.TPM_RC.SUCCESS) {
                    ...
                    activateCallback(new azure_iot_common_1.errors.SecurityDeviceError('Authorization session unable to be created.  RC value: ' + tss_js_1.TPM_RC[_this._tpm.getLastResponseCode()].toString()));
                }
                else {
                    ...
                    //
                    _this._tpm.withSession(tss.NullPwSession).PolicySecret(tss.Endorsement, policySession_1.SessIn.sessionHandle, null, null, null, 0, function (resp) {
                        debug('PolicySecret() returned ' + tss_js_1.TPM_RC[_this._tpm.getLastResponseCode()]);
                        if (_this._tpm.getLastResponseCode() !== tss_js_1.TPM_RC.SUCCESS) {
                            /*Codes_SRS_NODE_TPM_SECURITY_CLIENT_06_016: [If an error is encountered activating the identity key, the callback with be invoked with an `Error` of `SecurityDeviceError`.] */
                            activateCallback(new azure_iot_common_1.errors.SecurityDeviceError('Unable to apply the necessary policy to authorize the EK.  RC value: ' + tss_js_1.TPM_RC[_this._tpm.getLastResponseCode()].toString()));
                        }
                        else {
                            //
                            ...
                            //
                            _this._tpm.withSessions(tss.NullPwSession, policySession_1).ActivateCredential(TpmSecurityClient._srkPersistentHandle, TpmSecurityClient._ekPersistentHandle, credentialBlob, encodedSecret.secret, function (innerWrapKey) {
                                debug('ActivateCredential() returned ' + tss_js_1.TPM_RC[_this._tpm.getLastResponseCode()] + '; innerWrapKey size ' + innerWrapKey.length);
                                if (_this._tpm.getLastResponseCode() !== tss_js_1.TPM_RC.SUCCESS) {
                                    /*Codes_SRS_NODE_TPM_SECURITY_CLIENT_06_016: [If an error is encountered activating the identity key, the callback with be invoked with an `Error` of `SecurityDeviceError`.] */
                                    activateCallback(new azure_iot_common_1.errors.SecurityDeviceError('Unable to decrypt the symmetric key used to protect duplication blob.  RC value: ' + tss_js_1.TPM_RC[_this._tpm.getLastResponseCode()].toString()));
                                }
                                else {
                                    //
                                    ...
                                    //
                                    _this._tpm.withSession(tss.NullPwSession).Import(TpmSecurityClient._srkPersistentHandle, innerWrapKey, _this._idKeyPub, idKeyDupBlob, encWrapKey.secret, symDef, function (idKeyPrivate) {
                                        debug('Import() returned ' + tss_js_1.TPM_RC[_this._tpm.getLastResponseCode()] + '; idKeyPrivate size ' + idKeyPrivate.buffer.length);
                                        if (_this._tpm.getLastResponseCode() !== tss_js_1.TPM_RC.SUCCESS) {
                                            /*Codes_SRS_NODE_TPM_SECURITY_CLIENT_06_016: [If an error is encountered activating the identity key, the callback with be invoked with an `Error` of `SecurityDeviceError`.] */
                                            activateCallback(new azure_iot_common_1.errors.SecurityDeviceError('Unable to import the device id key into the TPM.  RC value: ' + tss_js_1.TPM_RC[_this._tpm.getLastResponseCode()].toString()));
                                        }
                                        else {
                                            //
                                            // Load the imported key into the TPM
                                            //
                                            _this._tpm.withSession(tss.NullPwSession).Load(TpmSecurityClient._srkPersistentHandle, idKeyPrivate, _this._idKeyPub, function (hIdKey) {
                                                debug('Load() returned ' + tss_js_1.TPM_RC[_this._tpm.getLastResponseCode()] + '; ID key handle: 0x' + hIdKey.handle.toString(16));
                                                if (_this._tpm.getLastResponseCode() !== tss_js_1.TPM_RC.SUCCESS) {
                                                    /*Codes_SRS_NODE_TPM_SECURITY_CLIENT_06_016: [If an error is encountered activating the identity key, the callback with be invoked with an `Error` of `SecurityDeviceError`.] */
                                                    activateCallback(new azure_iot_common_1.errors.SecurityDeviceError('Unable to load the device id key into the TPM.  RC value: ' + tss_js_1.TPM_RC[_this._tpm.getLastResponseCode()].toString()));
                                                }
                                                else {
                                                    //
                                                    ...
                                                    _this._tpm.allowErrors().withSession(tss.NullPwSession).EvictControl(tss.Owner, TpmSecurityClient._idKeyPersistentHandle, TpmSecurityClient._idKeyPersistentHandle, function () {
                                                        ...
                                                        //
                                                        _this._tpm.withSession(tss.NullPwSession).EvictControl(tss.Owner, hIdKey, TpmSecurityClient._idKeyPersistentHandle, function () {
                                                            if (_this._tpm.getLastResponseCode() !== tss_js_1.TPM_RC.SUCCESS) {
                                                                /*Codes_SRS_NODE_TPM_SECURITY_CLIENT_06_016: [If an error is encountered activating the identity key, the callback with be invoked with an `Error` of `SecurityDeviceError`.] */
                                                                activateCallback(new azure_iot_common_1.errors.SecurityDeviceError('Unable to persist the device id key into the TPM.  RC value: ' + tss_js_1.TPM_RC[_this._tpm.getLastResponseCode()].toString()));
                                                            }
                                                            else {
                                                                //
                                                                // Free the ID Key transient handle and the session object.  Doesn't matter if it "fails".  Go on at this point./
                                                                //
                                                                _this._tpm.FlushContext(hIdKey, function () {
                                                                    debug('FlushContext(TRANS_ID_KEY) returned ' + tss_js_1.TPM_RC[_this._tpm.getLastResponseCode()]);
                                                                    _this._tpm.FlushContext(policySession_1.SessIn.sessionHandle, function () {
                                                                        debug('FlushContext(POLICY_SESS) returned ' + tss_js_1.TPM_RC[_this._tpm.getLastResponseCode()]);
